import type { ContextCallbackParams, DatumCallbackParams, HighlightState, Listener, SelectionState, SeriesCallbackParams, Styler } from '../../chart/callbackOptions';
...
export interface AgHistogramSeriesTooltipRendererParams<TDatum = DatumDefault, TContext = ContextDefault> extends Omit<AgCartesianSeriesTooltipRendererParams<TDatum, TContext>, 'xKey' | 'yKey' | 'datum'>, AgHistogramSeriesBinParams<TDatum>, FillOptions, StrokeOptions {
    /** xKey as specified on series options. */
    readonly xKey: DatumKey<TDatum>;
    /** yKey as specified on series options. */
    readonly yKey?: DatumKey<TDatum>;
}
export interface AgHistogramSeriesLabelFormatterParams<TDatum = DatumDefault> extends AgHistogramSeriesOptionsKeys<TDatum>, AgHistogramSeriesOptionsNames, AgHistogramSeriesBinParams<TDatum> {
    /** The default label value that would have been used without a formatter. */
    readonly value: any;
}
export interface AgHistogramSeriesGetItemIdParams<TDatum = DatumDefault, TContext = ContextDefault> extends ContextCallbackParams<TContext>, AgHistogramSeriesBinParams<TDatum> {
}
export interface AgHistogramSeriesStyle extends FillOptions, StrokeOptions, LineDashOptions {
    /** Apply rounded corners to each bar. */
    cornerRadius?: PixelSize;
}
export interface AgHistogramSeriesItemStylerParams<TDatum = DatumDefault, TContext = ContextDefault> extends Omit<DatumCallbackParams<TDatum, HighlightState>, 'datum'>, ContextCallbackParams<TContext>, AgHistogramSeriesOptionsKeys<TDatum>, AgHistogramSeriesBinParams<TDatum>, Required<AgHistogramSeriesStyle> {
}
export interface AgHistogramSeriesStylerParams<TDatum = DatumDefault, TContext = ContextDefault> extends SeriesCallbackParams<HighlightState, SelectionState>, ContextCallbackParams<TContext>, AgHistogramSeriesOptionsKeys<TDatum>, Required<AgHistogramSeriesStyle> {
}
export interface AgHistogramSeriesThemeableOptions<TDatum = DatumDefault, TContext = ContextDefault> extends Omit<AgBaseCartesianThemeableOptions<TDatum, TContext>, 'selection'>, AgHistogramSeriesStyle {
...
export interface AgHistogramSeriesNodeClickEvent<TDatum = DatumDefault, TContext = ContextDefault> extends Omit<AgNodeClickEvent<'seriesNodeClick', TDatum, TContext>, 'datum' | 'datums'>, AgHistogramSeriesBinParams<TDatum> {
}
export interface AgHistogramSeriesNodeDoubleClickEvent<TDatum = DatumDefault, TContext = ContextDefault> extends Omit<AgNodeClickEvent<'seriesNodeDoubleClick', TDatum, TContext>, 'datum' | 'datums'>, AgHistogramSeriesBinParams<TDatum> {
}
...
export interface AgHistogramSeriesOptions<TDatum = DatumDefault, TContext = ContextDefault> extends Omit<AgBaseSeriesOptions<TDatum, TContext>, 'highlight' | 'selection' | 'listeners'>, AgBaseCartesianSeriesAxisOptions, AgHistogramSeriesOptionsKeys<TDatum>, AgHistogramSeriesOptionsNames, Omit<AgHistogramSeriesThemeableOptions<TDatum, TContext>, 'listeners'> {
