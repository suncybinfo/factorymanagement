...
type ExtendLiteralLeavesInner<T, V, E, P extends keyof T> = NonNullable<T[P]> extends Array<infer U> ? Array<ExtendLiteralLeaves<U, V, E>> | V : T[P] extends E ? T[P] | V : ExtendLiteralLeaves<T[P], V, E> | V;
...
type ChartOperation = {
    ...
    $isPackageType: Leaf<'community' | 'enterprise'>;
} | {
...
type ColorOperation = {
    ...
    $opacity: [Leaf<string>, Leaf<number>];
};
type FontOperation = {
    $rem: Leaf<number> | [Leaf<number>, Leaf<ThemeParam>];
};
...
type LogicOperation = {
    ...
    $some: [AnyLeaf | object, AnyLeaf];
} | {
    $every: [AnyLeaf | object, AnyLeaf];
} | {
